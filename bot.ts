...
const startInlineKeyboard = new InlineKeyboard().url(
  "Join My Channel",
  "https://t.me/BotzHub",
).url("Source", "https://github.com/xditya/mdiskDownloadLinks");
bot.command("start", async (ctx) => {
  await ctx.reply(
    `Hi ${
      ctx.from!.first_name
    }, I'm @${me.username}. I can give direct download links from any mdisk link you send me!`,
    {
...
bot.on("message", async (ctx) => {
  ...
  if (
    msg == undefined || msg?.startsWith("/") ||
    !(msg?.startsWith("https://mdisk.me"))
  ) return;
  ...
  const kbd = new InlineKeyboard().url("Download Now!", dl_url).url(
    "Updates",
    "https://t.me/BotzHub",
  );
  await ctx.api.editMessageText(
    reply.chat.id,
    reply.message_id,
    `Click the \"Download Now!\" button below to direct download from <code>${url}</code>`,
...
async function getLink(param: string) {
  const req_url =
    "https://diskuploader.entertainvideo.com/v1/file/cdnurl?param=" + param;
  try {
